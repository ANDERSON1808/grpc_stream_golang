...
func (c *testServiceClient) EnrollmentStudents(ctx context.Context, opts ...grpc.CallOption) (TestService_EnrollmentStudentsClient, error) {
	stream, err := c.cc.NewStream(ctx, &TestService_ServiceDesc.Streams[1], "/test.TestService/EnrollmentStudents", opts...)
	if err != nil {
		return nil, err
	}
...
func (x *testServiceGetStudentsPerTestClient) Recv() (*studenttpb.Student, error) {
	m := new(studenttpb.Student)
	if err := x.ClientStream.RecvMsg(m); err != nil {
		return nil, err
	}
	return m, nil
...
func (x *testServiceGetStudentsPerTestServer) Send(m *studenttpb.Student) error {
	return x.ServerStream.SendMsg(m)
}
